...
def fit(model, step_fn, val_fn, epochs, patience, tag, rel_tol=REL_TOL):
    ...
    for ep in range(epochs):
        ...
        for _ in range(STEPS_PER_EPOCH):
            idx = TRAIN_SUB[torch.randint(0, TRAIN_SUB.shape[0], (TRAIN_BS,), device=DEVICE)]
...
metric_rows = []
preds = {}
ALL = torch.arange(ntr, device=DEVICE)
TRAIN_SUB = ALL; TRAIN_BS = BATCH

for Nsub in N_GRID + [ntr]:
    TRAIN_SUB = PERM[:Nsub]; TRAIN_BS = min(BATCH, Nsub)
    ...
    with torch.no_grad():
        pred = pd_fno(fno_in_pure(te_v))
    metric_rows.append(("all", "puredata_fno", Nsub, rel_l2(pred, te_u)))
...
for name in MISSPECS:
    ...
    metric_rows.append((name, "prior_l2", -1, rel_l2(g_te, te_u)))
    ...
    for Nsub in N_GRID:
        ...
        fit(cb_fno, lambda idx: ((g_tr[idx] + cb_fno(fno_in_comb(tr_v[idx], g_tr[idx])) - tr_u[idx]) ** 2).mean(),
            ...
            DATA_EPOCHS, DATA_PATIENCE, "")
        with torch.no_grad():
            pred = g_te + cb_fno(fno_in_comb(te_v, g_te))
        metric_rows.append((name, "combined_fno", Nsub, rel_l2(pred, te_u)))
...
mpath = os.path.join(OUT_DIR, "pde1_metrics.txt")
with open(mpath, "w") as f:
    f.write("misspec\tmethod\tN\tvalue\n")
    for a, b, c, d in metric_rows:
        f.write(f"{a}\t{b}\t{c}\t{d:.6e}\n")
ppath = os.path.join(OUT_DIR, "pde1_preds.npz")
np.savez_compressed(ppath, x=x_np, u_true=te_u.detach().cpu().numpy(), **preds)

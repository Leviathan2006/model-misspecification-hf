...
for Nsub in N_GRID + [ntr]:
    ...
    pred = fno_pure_full(pd_fno, te_vi)
...
for name in MISSPECS:
    ...
    for Nsub in N_GRID:
        ...
        pred = fno_comb_full(cb_fno, te_vi, G_te)
...
mpath = os.path.join(OUT_DIR, "pde5_metrics.txt")
with open(mpath, "w") as f:
    f.write("misspec\tmethod\tN\tvalue\n")
    for a, b, c, d in metric_rows:
        f.write(f"{a}\t{b}\t{c}\t{d:.6e}\n")
ppath = os.path.join(OUT_DIR, "pde5_preds.npz")
np.savez_compressed(ppath, x=x_np, y=y_np, u_true=te_u.detach().cpu().numpy(), **preds)

...
HF_ZIP = "pde5_fractional_diffusion.zip"
...
FEAT = 2 * M_FOUR + 1
kx = torch.arange(Nx // 2 + 1, device=DEVICE).float()
...
VAL_TVALS = torch.rand(PHYS_BATCH_NL, NT_SUB, device=DEVICE) * T_END
...
def fracderiv(u, s):
    ...
    sym = (2.0 * PI * kx) ** (2.0 * s)
    return torch.fft.irfft(U * sym, n=Nx, dim=-1)
...
def res_local(name, u, ux, uxx, ut, x0):
    if name == "high":
        return ut - KAPPA_TRUE * uxx
    a = KAPPA_TRUE * (1.0 + 0.5 * torch.cos(2 * PI * x0))
    ap = -KAPPA_TRUE * PI * torch.sin(2 * PI * x0)
    return ut - a * uxx - ap * ux


def res_nonlocal(name, u, ut):
    if name == "low":
        return ut + 0.033 * fracderiv(u, 0.75)
    return ut + 0.03 * fracderiv(u, 0.65)
...
def nonlocal_residual(prior, name, vin, tvals):
    B = vin.shape[0]
    xg = x_row.view(1, 1, Nx).expand(B, NT_SUB, Nx)
    tg = tvals.view(B, NT_SUB, 1).expand(B, NT_SUB, Nx)
    coords = torch.stack([xg, tg], dim=-1).reshape(B, NT_SUB * Nx, 2).clone().requires_grad_(True)
    u = prior.pointwise(vin, coords)
    ut = torch.autograd.grad(u.sum(), coords, create_graph=True)[0][..., 1]
    u = u.reshape(B, NT_SUB, Nx); ut = ut.reshape(B, NT_SUB, Nx)
    return res_nonlocal(name, u, ut)
...
def train_prior(name):
    ...
    if name in ("low", "med"):
        def step():
            idx = torch.randint(0, ntr, (PHYS_BATCH_NL,), device=DEVICE)
            tvals = torch.rand(PHYS_BATCH_NL, NT_SUB, device=DEVICE) * T_END
            vin = vnorm(tr_v[idx])
            ic = ((prior.eval_grid(vin, ic_coords) - tr_v[idx]) ** 2).mean()
            return (nonlocal_residual(prior, name, vin, tvals) ** 2).mean() + W_IC * ic

        def val():
            idx = torch.arange(min(PHYS_BATCH_NL, nval), device=DEVICE)
            vin = vnorm(va_v[idx])
            ic = ((prior.eval_grid(vin, ic_coords) - va_v[idx]) ** 2).mean()
            return ((nonlocal_residual(prior, name, vin, VAL_TVALS[:idx.shape[0]]) ** 2).mean() + W_IC * ic).item()
    else:
        def step():
            ...
            ic = ((prior.eval_grid(vin, ic_coords) - tr_v[idx]) ** 2).mean()
            return (res_local(name, u, ux, uxx, ut, coll[..., 0]) ** 2).mean() + W_IC * ic
        ...
        def val():
            ...
            u, ux, uxx, ut = derivs(prior, vin, coll)
            ic = ((prior.eval_grid(vin, ic_coords) - va_v[idx]) ** 2).mean()
            return ((res_local(name, u, ux, uxx, ut, coll[..., 0]) ** 2).mean() + W_IC * ic).item()
...
def phys_residual(prior, name):
    tot, cnt = 0.0, 0
    if name in ("low", "med"):
        for b in range(0, nte, PHYS_BATCH_NL):
            e = min(b + PHYS_BATCH_NL, nte); vb = te_v[b:e]
            tvals = torch.rand(vb.shape[0], NT_SUB, device=DEVICE) * T_END
            r = nonlocal_residual(prior, name, vnorm(vb), tvals)
        ...
        for b in range(0, nte, PHYS_BATCH):
            ...
            r = res_local(name, u, ux, uxx, ut, coll[..., 0])
            tot += (r ** 2).sum().item(); cnt += r.numel()
    return tot / cnt
...
def Mchan(u):
    B = u.shape[0]
    return torch.stack([gnorm(u), X2D[None].expand(B, Nt, Nx), T2D[None].expand(B, Nt, Nx)], dim=1)
...
for name in MISSPECS:
    ...
    def s():
        ...
        return ((Mpsi(Mchan(G_tr[idx])) - tr_u[idx]) ** 2).mean()
    ...
    def v():
        with torch.no_grad():
            ...
            for b in range(0, nval, FNO_BATCH):
                ...
                tot += ((Mpsi(Mchan(G_va[b:e])) - va_u[b:e]) ** 2).sum().item()
            return tot / (nval * Nt * Nx)
...
mpath = os.path.join(OUT_DIR, "pde4_diag_metrics.txt")
with open(mpath, "w") as f:
    f.write("misspec\tfeature\tvalue\n")
    for a, b, c in metric_rows:
        f.write(f"{a}\t{b}\t{c:.6e}\n")
tpath = os.path.join(OUT_DIR, "pde4_diag_transfer.npz")

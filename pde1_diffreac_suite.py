...
def fit(model, step_fn, val_fn, epochs, patience, tag):
...
for name in MISSPECS:
    ...
    results[(name, "prior_l2")] = rel_l2(g_te, te_u)
    print(f"{name} phys_residual {results[(name, 'phys_residual')]:.6e} prior_l2 {results[(name, 'prior_l2')]:.6e}", flush=True)

    pd_don = DeepONet(Nx, SMALL_BH, SMALL_P, SMALL_TH).to(DEVICE)
    fit(pd_don, lambda idx: ((pd_don(vnorm(tr_v[idx]), x_grid) - tr_u[idx]) ** 2).mean(),
        lambda: ((pd_don(vnorm(va_v), x_grid) - va_u) ** 2).mean().item(),
        DATA_EPOCHS, DATA_PATIENCE, f"{name} puredata_deeponet")
    with torch.no_grad():
        results[(name, "puredata_deeponet")] = rel_l2(pd_don(vnorm(te_v), x_grid), te_u)

    pd_fno = FNO1d(2, FW, FM, FL).to(DEVICE)
    fit(pd_fno, lambda idx: ((pd_fno(fno_in_pure(tr_v[idx])) - tr_u[idx]) ** 2).mean(),
        lambda: ((pd_fno(fno_in_pure(va_v)) - va_u) ** 2).mean().item(),
        DATA_EPOCHS, DATA_PATIENCE, f"{name} puredata_fno")
    with torch.no_grad():
        results[(name, "puredata_fno")] = rel_l2(pd_fno(fno_in_pure(te_v)), te_u)

    cb_don = DeepONet(2 * Nx, SMALL_BH, SMALL_P, SMALL_TH).to(DEVICE)
    fit(cb_don,
        lambda idx: ((g_tr[idx] + cb_don(torch.cat([vnorm(tr_v[idx]), gnorm(g_tr[idx])], 1), x_grid) - tr_u[idx]) ** 2).mean(),
        lambda: ((g_va + cb_don(torch.cat([vnorm(va_v), gnorm(g_va)], 1), x_grid) - va_u) ** 2).mean().item(),
        DATA_EPOCHS, DATA_PATIENCE, f"{name} combined_deeponet")
    with torch.no_grad():
        results[(name, "combined_deeponet")] = rel_l2(g_te + cb_don(torch.cat([vnorm(te_v), gnorm(g_te)], 1), x_grid), te_u)

    cb_fno = FNO1d(3, FW, FM, FL).to(DEVICE)
    fit(cb_fno, lambda idx: ((g_tr[idx] + cb_fno(fno_in_comb(tr_v[idx], g_tr[idx])) - tr_u[idx]) ** 2).mean(),
        lambda: ((g_va + cb_fno(fno_in_comb(va_v, g_va)) - va_u) ** 2).mean().item(),
        DATA_EPOCHS, DATA_PATIENCE, f"{name} combined_fno")
    with torch.no_grad():
        results[(name, "combined_fno")] = rel_l2(g_te + cb_fno(fno_in_comb(te_v, g_te)), te_u)
...
path = os.path.join(OUT_DIR, "pde1_metrics.txt")
